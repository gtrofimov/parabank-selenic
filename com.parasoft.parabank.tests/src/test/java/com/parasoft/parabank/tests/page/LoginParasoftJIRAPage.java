...
public class LoginParasoftJIRAPage {

	@FindBy(name = "os_username")
	private WebElement osUsernameField;

	@FindBy(name = "os_password")
	private WebElement osPasswordField;

	private WebDriver driver;

	private static final int DEFAULT_WAIT_FOR_ELEMENT_TIMEOUT = 15;

	private static final String[] TITLE_WORDS = { "Log", "in", "Parasoft", "JIRA" };

	public LoginParasoftJIRAPage(WebDriver driver) {
		this.driver = driver;
	...
	public void setOsUsernameField(String text) {
		waitFor(osUsernameField).clear();
		osUsernameField.sendKeys(text);
	}

	public void setOsPasswordField(String text) {
		waitFor(osPasswordField).clear();
		osPasswordField.sendKeys(text);
	}
